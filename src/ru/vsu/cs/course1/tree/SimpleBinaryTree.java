...
public class SimpleBinaryTree<T> implements BinaryTree<T> {
    ...
    public class SimpleTreeNode implements BinaryTree.TreeNode<T> {
        public T value;
    ...
    public static TreeNode findMinValueInTree(TreeNode node, TreeNode minValueNode) {
        if (node == null) {
            return null;
        }
        TreeNode leftNode = findMinValueInTree(node.getLeft(), minValueNode);
        TreeNode rightNode = findMinValueInTree(node.getRight(), minValueNode);
        if(leftNode == null || rightNode == null) {
            return  null;
        }
        if ((float)leftNode.getValue() < (float)minValueNode.getValue()) {
            minValueNode = leftNode;
        }
        if ((float)rightNode.getValue() < (float) minValueNode.getValue()) {
            minValueNode = rightNode;
        }
        return minValueNode;
    }
