...
public class BinaryTreeAlgorithms {
    ...
    public static float findMInValue(BinaryTree tree1) { // здесь используется итератор для прохода по дереву, ищу наименьшее значение
        Iterator iterator1 = byLevelValues(tree1.getRoot()).iterator(); //итератор для дерева
        float minValue = (float) tree1.getRoot().getValue();
        while (iterator1.hasNext()) {//цикл работает, пока есть следующая нода
            float tmp = (float)iterator1.next();
            if (tmp < minValue) {
                minValue = tmp;
            }
        }
